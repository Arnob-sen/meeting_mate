...
export const meetingsService = {
  ...
  async getAllMeetings(): Promise<Meeting[]> {
    try {
      const response = await apiClient.get<Meeting[]>(API_ENDPOINTS.MEETINGS.GET_ALL);
      
      // Check if response is HTML (ngrok warning page)
      if (typeof response.data === 'string' && (response.data as string).includes('<!DOCTYPE html>')) {
        console.error('Received HTML instead of JSON. This might be ngrok warning page.');
        throw new Error('Received HTML response. Please check ngrok configuration.');
      }
      ...
      if (data && Array.isArray((data as Record<string, unknown>).data as Meeting[])) {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response?.data && typeof error.response.data === 'string' && error.response.data.includes('<!DOCTYPE html>')) {
          throw new Error('Received HTML response from server. This might be ngrok warning page. Please check your ngrok configuration.');
        }
        
        throw new Error(
          error.response?.data?.message || `Failed to fetch meetings: ${error.response?.statusText || error.message}`,
        );
      }
      throw error;
    }
